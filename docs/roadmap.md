...
- [x] **フォントのローカル同梱**（`@fontsource/ibm-plex-{sans,mono}` の latin サブセットを
  main.tsx で import。index.html の Google Fonts/CSP CDN 許可を撤去＝完全オフライン）
...
- [x] Pixiv Fanbox アダプタ（`src/main/services/fanbox/`、正規化テスト付き）
  - [x] **実 API 検証済み**（`scripts/probe-fanbox.cjs`。checkAuth/列挙/投稿/画像 DL 通し確認。
    checkAuth の形違いと listPosts のページング誤りを修正）
  - [x] リトライ / バックオフ（共通エンジン M4 で対応済み）
- [x] ci-en アダプタ（`src/main/services/cien/`、HTML スクレイプ＋パーステスト付き）
  - [x] **実 API 検証済み** 2026-06-08（`scripts/probe-cien.cjs`。checkAuth（/mypage の
    user-menu リンク）→ 購読クリエイター列挙（/mypage/subscription）→ 記事ページング
    （/creator/<id>/article?page=N）→ 限定公開添付（media.ci-en.jp/private、署名付き
    upload/ オリジナル）DL を **HTTP 200 で通し確認**）
- [~] Patreon アダプタ（`src/main/services/patreon/`、正規化テスト付き）
  - [x] **checkAuth 実 API 検証済み** 2026-06-08（`scripts/probe-patreon.cjs`。
    `/api/current_user` → 200 `{data:{id,type:"user"}}`。current_user の
    memberships/pledges リレーション存在を確認）
  - [ ] **VERIFY:** 支援先列挙・投稿/メディア DL は JSON:API 構造ベース。検証アカウントに
    アクティブな pledge が無かったため、課金済みアカウントでの実取得確認が未了
- [x] Fantia アダプタ（**実 API 検証済み** 2026-06-08。checkAuth=current_user.id、me/fanclubs、HTML スクレイプ+CSRF、画像DL確認）
