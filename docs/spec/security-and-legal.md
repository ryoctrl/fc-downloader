...
アプリが対象サイトへ**自動で発行するリクエスト**（支援先・投稿の列挙、ファイル取得）は、
各サービスへ過度な負荷をかけないよう **per-service の最小間隔スロットル** を必ず通す。

- 実装: `src/main/session/throttle.ts` の `awaitPoliteSlot(serviceId)`。同一サービスへの
  リクエスト**開始**を最低 `MIN_GAP_MS`（既定 300ms）間隔に整える。`requestFor`（メタデータ）と
  `downloadToFile`（ファイル）の**両方**がこのゲートを通過する。
- **同時実行数とは独立**: ダウンロード並列数を N にしても、リクエストは 300ms ごとに 1 本ずつ
  解放されるため、実効レートは概ね `1000 / MIN_GAP_MS`（≒ 3 req/s）/サービスに収まる。
  大きなファイル転送はこの間隔より十分長いため、実用上スループットはほぼ犠牲にならない。
- 大量列挙（HTML ページ走査・`post.info` 連打など）のバースト送信を避けるのが主目的。
