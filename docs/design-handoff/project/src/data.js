/* fc-downloader — mock data layer (plain JS, attached to window.FC) */
(function () {
  ...
  const TITLES = [
    '線画 + PSD セット', '今月の差分まとめ', 'ラフ画アーカイブ', '立ち絵 表情差分',
    '背景イラスト 4K', 'メイキング動画', 'ボイス付き動画', '設定資料 zip',
    '壁紙パック 12月', 'ミニ漫画 第3話', 'スケッチログ', 'キャラデザ案 v2',
    '塗り工程タイムラプス', '限定イラスト', 'コンセプトアート集',
  ];
  const TAGS = ['イラスト', '差分', 'PSD', '動画', '限定', 'ラフ', '背景', '壁紙', 'メイキング', 'ボイス'];
  const TYPES = ['image', 'image', 'image', 'image', 'file', 'file', 'video'];

  // ── build posts ──
  let pid = 8000;
  const POSTS = [];
  SERVICES.forEach((svc, si) => {
    (CREATORS[svc.id] || []).forEach((cr, ci) => {
      ...
      for (let k = 0; k < n; k++) {
        ...
        const files = type === 'video' ? range(rng, 1, 2)
          : type === 'file' ? range(rng, 1, 4) : range(rng, 3, 28);
        const sizeMB = type === 'video' ? range(rng, 80, 720)
          : type === 'file' ? range(rng, 12, 240) : range(rng, 4, 60);
        const r = rng();
        const status = r < 0.62 ? 'done' : r < 0.74 ? 'partial' : 'new';
        const ntags = range(rng, 1, 3);
        const tags = [];
        for (let t = 0; t < ntags; t++) { const tg = pick(rng, TAGS); if (!tags.includes(tg)) tags.push(tg); }
        pid += range(rng, 3, 40);
        POSTS.push({
          id: pid,
          service: svc.id,
  ...
  window.FC = {
    SERVICES, CREATORS, POSTS, TAGS,
    countsForService, fmtSize,
