...
1. `fc-downloader-x.x.x-x64.exe` をダウンロードして**そのまま実行**。
2. 「Windows によって PC が保護されました」と出たら **「詳細情報」→「実行」**でインストールを進めます。
3. インストール後の本体は警告なしで起動します。

**インストール不要版（ポータブル）**

1. `fc-downloader-x.x.x-portable.zip` をダウンロード。
2. **zip を右クリック →「プロパティ」→「許可する」にチェック → OK**。
3. zip を**展開**して、中の `.exe` を実行。

> 💡 表示される警告はウイルスではなく、安全のための署名（コード署名）を省いた配布のため出ます。
...
本アプリの**ソースコード**は [MIT License](LICENSE) で公開しています。

> ⚠️ MIT が適用されるのは**このアプリのコード**だけです。アプリで**ダウンロードしたコンテンツ**の
> 権利は各クリエイター／サービスに帰属します。本ライセンスはそれらの再配布・利用を許諾するものでは
> ありません（上記「免責事項」を参照）。
